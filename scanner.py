...
def transcribe_episode(episode):
    """Full pipeline: download YouTube audio → upload → transcribe."""
...
def generate_basic_summary(episode, transcript_data):
    ...
    key_moment = matching[0] if matching else {
        ...
        "timestamp_seconds": 0,
        "context": "Automated detection — manual review recommended",
    }
...
<p><a href="{episode['url']}">▶ Watch Episode</a> |
<a href="{dropbox_links['summary']}">📄 Summary</a> |
<a href="{dropbox_links['transcript']}">📄 Full Transcript</a></p>"""
